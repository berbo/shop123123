...
//            }
        ...
        return $this->wp_print('921517061', $content, $time);
    }

    //飞鹅打印机打印采购单
    public function printer_4($startTime,$endTime)
    {
        date_default_timezone_set("Asia/Shanghai");
        $order = $this->order;
        $addtime = date('Y-m-d H:i:s', $order['addtime']);
        $goods_list = $this->getGoodsSaleTopList($startTime, $endTime, $mchId = 0, $limit = 10);
        $order_detail_idarray="";
        $array=array();
        $s=0;
        for($i=0;$i<count($goods_list);$i++) {
            $name = $goods_list[$i]['name'];
            $attr = $goods_list[$i]['attr'];
            $num = $goods_list[$i]['num'];
            $pic = $goods_list[$i]['pic'];
            $price = $goods_list[$i]['price'];
            $order_detail_idarray = $order_detail_idarray . ',' . $goods_list[$i]['id'] .
            ...
            if (is_numeric($found_key)) {
                ...
                $array[$found_key]['order_detail_id'] =$array[$found_key]['order_detail_id'].','.$goods_list[$i]['id'];
            } else {
                ...
                $array[$s]['pic'] = $pic;
                $array[$s]['price'] = $price;
                $array[$s]['order_detail_id'] = $goods_list[$i]['id'];
                $s++;
            }
        }
        $time = "1";//打印联数
        ...
        if ($this->setting->is_attr == 0) {
            $content .= "------------------------------<BR>";
            $content .= "<CB>采购单</CB><BR>";
            $content .= "------------------------------<BR>";
            for($a=0;$a<count($array);$a++)
            {
                $good_attr=json_decode($array[$a]['good_attr'],ture);
                $content .= $this->r_str_pad($array[$a]['good_name'],20)."<BR>" .$array[$a]['num']."<BR>".$good_attr[0]['attr_group_name'].":".$good_attr[0]['attr_name'].'<BR>';
                $content .= "--------------------------<BR>";
            }
//            foreach ($goods_list as $k => $v) {
//                $price = round($v['total_price'] / $v['num'], 2);
...
//            }
        ...
        $this->printer_setting['user']="410915403@qq.com";
        $this->printer_setting['ukey']="WsTTLspmRbM2MR2Y";
        return $this->wp_print('921517061', $content, $time);
    }
    //飞鹅打印机打印发货单
    public function printer_5($order_ids)
    {

        $this->order_id=$order_ids;
        $this->setting = PrinterSetting::findOne(['store_id' =>'1']);//打印设置
        if (!$this->setting) {
            return false;
        }
        $type = json_decode($this->setting->type, true);
...
//        $order = Order::findOne(['id' => $this->order_id, 'is_delete' => 0]);
        ...
        $addtime = date('Y-m-d H:i:s', $order['addtime']);
//        $goods_list = $this->getOrderGoodsList($order->id);
        $goods_list = $OrderDetaillistresponse;
        ...
        return $this->wp_print('921522949', $content, $time);
...
//                'o.is_send'=>Model::IS_DELETE_FALSE,
//                'g.mch_id' => $mchId,
//                'o.mch_id' => $mchId,
...
//            ->asArray()->all();
        ...
        return $query->select('o.order_no,od.attr,g.name,od.num,od.id,od.pic,g.price')
