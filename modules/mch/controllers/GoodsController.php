...
class GoodsController extends Controller
{
    ...
    public function actionGoods($keyword = null, $status = null)
    {
        ...
        {
            $res['list'][$i]['virtual_sales']=$this->getSalesVolume($res['list'][$i]['id']);
        }
     ...
     * 获取商品销量
     */
    public function getSalesVolume($id)
    {
        $res = OrderDetail::find()->alias('od')
            ->select('SUM(od.num) AS sales_volume')
            ->leftJoin(['o' => Order::tableName()], 'od.order_id=o.id')
            ->where(['od.is_delete' => 0, 'od.goods_id' =>$id, 'o.is_delete' => 0, 'o.is_pay' => 1,])
            ->asArray()->one();
        return intval($res['sales_volume']);
    }
